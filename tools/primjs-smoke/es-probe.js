// EXPERIMENT (see README.md) — the language-level half of the compatibility
// question. PrimJS advertises "Fully supporting ES2019"; this repo's esbuild
// preset emits `target: "es2020"` (js/esbuild/preset.mts), so a bundle is
// allowed to contain constructs a full spec year past what PrimJS claims. The
// production bundle booting proves the features it HAPPENS to use are present;
// it says nothing about the ones a consumer's app may use tomorrow.
//
...
// closures, one unsupported *syntax* (PrimJS has no `??=`) is a parse error for
// the whole file, and the run dies at line 1 reporting nothing. As strings, a
// missing operator is one MISSING row like any other and the other 25 answers
// still come back. A probe that can only report "everything" or "nothing" is
// not a probe.
//
// Run with either qjs CLI built by build-hosts.sh:
//   ./out/qjs-qjsng  es-probe.js
//   ./out/qjs-primjs es-probe.js
const checks = [
