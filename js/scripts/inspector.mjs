...
<div class=cols>
  ...
  <div class=col><h2>ERRORS</h2><pre id=errors></pre></div>
</div>
<script>
...
async function tick(){
  try{
    ...
    document.getElementById('logs').innerHTML = (s.logs||[]).map(l =>
      '<div class=log>'+esc(l)+'</div>').join('');
    document.getElementById('errors').innerHTML = (s.errors||[]).map(e =>
      '<div class=err>'+esc(e.message)+
      (e.componentStack ? '<div class=stack>'+esc(e.componentStack)+'</div>' : '')+
      '</div>').join('') || '<span class=muted>(no errors)</span>';
  }catch(e){}
}
