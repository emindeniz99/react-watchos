 ...
 * @param {{ ios?: { bundleIdentifier?: string, appleTeamId?: string } }} config
 * @param {ReactWatchOptions | undefined} options
 * @returns {import("./targetConfig.js").ResolvedOptions}
 */
 ...
 * @param {import("./targetConfig.js").ResolvedOptions} opts
...
function withEasAppExtensions(config, opts) {
  ...
  /** @param {{ targetName: string, bundleIdentifier: string,
   *   entitlements: Record<string, unknown> }} entry */
  const upsert = (entry) => {
    const i = ios.appExtensions.findIndex(
      (/** @type {{ targetName?: string }} */ e) =>
  ...
  const bundleIdFor = (/** @type {string} */ suffix) =>
