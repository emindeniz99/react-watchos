...
| 2 | Plugin sets `WKRunsIndependentlyOfCompanionApp` so a standalone watch app needs no manual Xcode step | ✅ **Done + hardened.** The plugin already defaulted the key and auto-applied it (prebuild→postprebuild merge) — so the "manual step" was already closed. Reading the code surfaced a *latent* issue instead: it was **hard-coded `true`**, and watch-app independence is **irreversible after an App Store upload** (Apple), so it silently made an unrevertable choice. Promoted to a first-class `independent` option (default `true`, matching this framework's standalone premise) gated so a companion-dependent app (`independent:false`) omits the key, with the irreversibility documented. | **High** — a safety fix the secondhand analysis missed |
