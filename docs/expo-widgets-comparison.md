...
|---|---|---|---|
| 1 | `onPress`/control handler **returns new state**; the runtime persists + reloads | `IntentHandler` returns `void` ([intents.ts:9](../js/src/intents.ts)); handlers must remember to call `publishWidgets()` ([demo/intents.ts:12](../js/demo/intents.ts)) | **High** — removes a real footgun (a forgotten republish silently no-ops the UI) |
