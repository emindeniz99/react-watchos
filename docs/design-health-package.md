...
| `healthLiveUpdates` | `startHealthUpdates` — the first SUBSCRIPTION in this package (`HKAnchoredObjectQueryDescriptor`, watchOS 8.5): a screen showing today's steps or the current heart rate updates itself instead of polling. Three decisions carry it. **The start rides `invoke`, the delivery rides the event channel** — a HealthKit start is fallible (no HealthKit, an authorization round trip) and the `sensor` direct method has no reply path at all, which is the wart this deliberately does not repeat. **New samples only:** `anchor: nil` means "everything matching, then updates", so the bound is the PREDICATE — an open-ended window from the subscribe instant, whose default overlap rule (`endDate >= start`) keeps the late-saved *straddling* sample a live steps screen exists to show — it is the sample's interval that decides, never its save time — while leaving history to `queryHealthSamples`. **Coalesced by merging, not dropping and not pacing:** every push is a bridge crossing plus a synchronous commit, so pushes are floored at `minIntervalMs` (default 1000, ceiling 60000) — and batches that arrive inside the floor are HELD IN THE BRIDGE and merged into one push, because unlike `workout.metrics` (level state) a sample stream is edge-triggered and a dropped batch is data nobody can get back. Held in the bridge rather than left unconsumed in Apple's `Results` sequence, whose buffering policy is documented nowhere; and merged rather than paced, or the knob would add latency without removing a single render. |
...
| `healthHourlyBuckets` | `queryHealthHourlyStatistics` (2026-08-22) — a METHOD, not the `intervalComponents` parameter the deferral priced at one line, because `queryHealthStatistics` shares the request shape and a bucket-size knob there would be a field the scalar query has to ignore; "the method name is the granularity" survives as a rule with two members instead of a pretend-union with one. The deferral's two contract questions got their answers where Linux can test them: the **ceiling** was never a daily one — it is the family's ONE 1000-**bucket** rule, counted in hours by `decodeHourly` (~41 days; the refusal message points a wider chart at the daily query) — and the **anchor** stays the caller's `startMs`, where the hour stride actually *dissolves* the recorded time-zone worry: hour steps are uniform 3 600 000 ms (DST moves labels, never hour lengths), so bucket *n* is `startMs + n·3600000` on every watch, and the only honest caveat is the JSDoc's "a local day is 23 or 25 hourly buckets on a DST day". Implementation is the daily query with a different stride — literally: both wrappers hand `bucketedStatistics` a `DateComponents` and everything else (contiguous `enumerateStatistics`, `containsBucketStart` boundary rule, the statistic legality) is the same shared code, pinned by the guards scan and the producer-key scan on the shared body. |
| `healthUpdateDeletions` | Shipped 2026-08-22, and the deferral was right about the price: sample IDENTITY on the wire. `HealthSample` gained `id` (`HKObject.uuid`, watchOS 2.0 — re-verified from the docs JSON, as were `HKDeletedObject`/`.uuid` at 2.0 and `Result.deletedObjects` at 8.5), on the SHARED row shape rather than a live-only variant, because "a live row and a `queryHealthSamples` row mean the same thing" is the property that makes a deletion actionable at all — a subscriber seeds its buffer from history and retracts against it. Deletions ride the live stream as `deletedIds` beside `samples` (same buffer, same `minIntervalMs` floor, same merge — so an add and its own deletion coalesced into one push net out to gone instead of arriving out of order), and reach JS through a per-subscriber `onDeleted` callback rather than through `HealthUpdate`: a retraction is not a sample, and folding it in would have cost the sample handler its `latest` guarantee (an update would then exist with nothing added) for an event most screens ignore. What no sim can prove — a real `HKDeletedObject` naming the uuid the add carried — is device-owed; `HKDeletedObject` has no public initializer and the sim build is signed without the `healthkit` entitlement. |
...
| `workoutHistoryDetail` | The rest of an `HKWorkout`: `metadata`, `device`, `sourceRevision`, `workoutEvents` (pause/lap markers), the full `allStatistics` map, and `HKWorkoutActivity` multisport segments | `queryWorkoutHistory` ships the row a LIST renders and nothing more — id, start/end, duration, activity, energy, distance. Everything here is *detail-view* surface, and each field is a wire cost paid by every row of every list; a watch showing five workouts would carry it and render none of it. A later `getWorkout(id)`-shaped read can add them without moving the shipped shape, and multisport stays deferred on the same evidence as the live-session side (no consumer in the prior-art survey ships it). |
| `workoutHistoryFilter` | An `activityType` filter on `queryWorkoutHistory` (`HKQuery.predicateForWorkouts(with:)`) | The one real argument for it is recorded rather than waved away: the cap applies to the WINDOW, so filtering client-side cannot recover "my last five runs" from a window full of walks. What it costs is a compound predicate plus an activity-name refusal that would have to live in the watchOS-only bridge (Support is Foundation-only and has no `HKWorkoutActivityType`), i.e. a rule Linux cannot test — so v1 documents the interaction in the JSDoc ("ask for more than five") instead of shipping an untestable refusal. One optional field when a caller hits it. |
| `activityRingsPaused` | `HKActivitySummary.isPaused` on the `queryActivitySummaries` row | watchOS **11.0**, above the v10 floor — it would be this family's first `@available` gate, in a package that has none. It is also the least renderable field of the three: it says the rings were paused *at some point*, not for how long, so a screen can show a badge and nothing else. One optional field behind one gate when the floor moves. *Re-verified 2026-08-22 against the docs JSON (rule 2, before the hourly/deletions wave took its neighbours): `isPaused` is still watchOS 11.0, not beta, and still the only paused-adjacent member on `HKActivitySummary` — so the verdict stands unchanged rather than riding along.* |
| `activityRingsBackfill` | A convenience that fills the days HealthKit returned no summary for | Deliberately not done: a day with no summary is a day the watch was off the wrist, and a synthesized `0 / 500` row is indistinguishable, on a chart, from a day the user genuinely moved nothing. `queryActivitySummaries` returns only the days that exist and labels every row with its `date`; deciding what a gap means is the app's call, not the bridge's. |
| `activityRingsGoalHistory` | Reporting *when* a goal changed, rather than per-day goals | Not a thing HealthKit exposes: `HKActivitySummary` carries the goal that day was scored against, and that per-day value is already what a ring needs. A "you raised your move goal on the 14th" story is derivable from the rows a caller already has. |
...
| — | `HKQueryOptions` (`.strictStartDate`/`.strictEndDate`) | Not exposed; default `[]`, which Apple documents as `endDate >= start AND startDate < end` — OVERLAP. Academic for the near-instantaneous quantity samples, and NOT for `queryWorkoutHistory`: an hour-long workout can overlap two windows, so its JSDoc states the overlap rule instead of the `[startMs, endMs)` framing the other reads use. Overlap is also the better default there — a hike the window cut in half is still a hike the list should show. |
| — | An `HKActivitySummaryQuery` (the callback class) or a live rings subscription | `queryActivitySummaries` uses the `HKActivitySummaryQueryDescriptor` (watchOS 8.5) like every other read here — the class's docs JSON carries no availability at all. The "no channel for it" half of this row is now out of date and left as the record: `startHealthUpdates` built that channel. What still stands is the *shape* — an activity summary is not a sample, so there is no anchored query for one, and streaming rings would mean the callback class's update handler with its own off-main hop and its own `stop(_:)`. A rings screen re-reads on foreground for a fraction of that. |
| `healthUpdateVocabulary` | Live streams for sleep stages or the rings | Both are outside `HKAnchoredObjectQueryDescriptor`'s reach as this package uses it: it is a SAMPLE query over the fourteen-type quantity vocabulary, whose per-type unit table is exactly what lets a live row and a `queryHealthSamples` row mean the same thing. A sleep stream is a screen nobody is awake to read, and the rings have the row above. *Re-checked 2026-08-22 when `healthUpdateDeletions` left this table: neither half moves. The rings are structurally out — an `HKActivitySummary` is not a sample, so there is no anchored query to arm (the class row above already records it) — and sleep is a deliberate decline that deletions shipping does not weaken; forcing a sleep stream would trade the quantity vocabulary's unit-table guarantee for a screen with no audience.* |
| — | `swimmingLocationType` / `lapLength`, `HKWorkoutActivity` multisport, `startEventUpdates`/`CMPedometerEvent`, `CMMotionActivityManager` | Additive surface with no caller yet. `CMMotionActivityManager` in particular needs no new grant — same consent — if it is ever wanted. **`heartRateVariabilitySDNN` left this row 2026-08-20:** the caller arrived — a recovery view charts HRV against resting heart rate — so it shipped as a read quantity type alongside `restingHeartRate`. Wire unit `"ms"` via `HKUnit.secondUnit(with: .milli)` (seconds would report 0.045 where the Health app shows 45), DISCRETE so `sum` is refused before the query runs, and watchOS 4.0 so the family stays gate-free. See the status.md HealthKit-reads row. |
...
and limit rules, the daily- and hourly-bucket ceilings and the bucket-boundary
rule, the
