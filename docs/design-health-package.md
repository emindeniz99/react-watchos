...
| `activityRingsPaused` | `HKActivitySummary.isPaused` on the `queryActivitySummaries` row | watchOS **11.0**, above the v10 floor — it would be this family's first `@available` gate, in a package that has none. It is also the least renderable field of the three: it says the rings were paused *at some point*, not for how long, so a screen can show a badge and nothing else. One optional field behind one gate when the floor moves. |
...
| `healthUpdateDeletions` | Report `deletedObjects` on the live stream | The anchored result carries deletions and the bridge drops them. Acting on one needs a sample IDENTITY on the wire, which no read here carries (`HealthSample` is `{startMs, endMs, value, unit}`), so it would widen the shared row shape for a case no screen has asked for — a user deleting a sample in the Health app while a live screen is open. |
| `healthUpdateVocabulary` | Live streams for sleep stages or the rings | Both are outside `HKAnchoredObjectQueryDescriptor`'s reach as this package uses it: it is a SAMPLE query over the fourteen-type quantity vocabulary, whose per-type unit table is exactly what lets a live row and a `queryHealthSamples` row mean the same thing. A sleep stream is a screen nobody is awake to read, and the rings have the row above. |
