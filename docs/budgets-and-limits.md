...
|---|---|---|---|---|---|---|
| 1 | **App JS bundle** | 2 MB min (demo ~180 KB) | [`js/scripts/config.ts`](../js/scripts/config.ts) `budgetKB` | lib-internal (sanity ceiling) | none directly — cost is **flash + app heap + OTA**, not boot parse (prod ships bytecode) | the real ceiling is the **OTA cap (3 MB, #7)** — above it a bundle ships in the binary but can't be OTA-updated. Raised to 2 MB (2026-07-05): since prod ships `.qbc`, boot parse is a non-issue, so this is a loose sanity ceiling with OTA margin, not a tight tripwire. >3 MB needs raising `maxOTABundleBytes` too |
| 2 | **Widget JS bundle** | 100 KB min (demo ~27 KB) | `config.ts` `budgetKB` | lib-internal (regression tripwire) | the **~30 MB widget-extension RSS** is the real wall — [§ Widget memory](#widget-memory--the-30-mb-story) | kept **far tighter than the app**: the widget bytecode loads into the 16 MB widget heap (#4), so size trades against MEMORY. Lowered 1 MB → 100 KB (2026-08-21) when widget timelines stopped rendering through react-reconciler ([`staticRender.ts`](../js/src/staticRender.ts)): the demo bundle went 153,362 → 27,870 B, since the reconciler + scheduler + adapter were 83.8% of it for a one-shot static render. 100 KB ≈ 3.5× the bundle — room for real widget code, while a stray import dragging the ~121 KB reconciler back in now FAILS instead of hiding under a 1 MB ceiling |
