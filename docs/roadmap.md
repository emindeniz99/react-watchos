...
| react-compiler.test flake under full parallel load | **watch item, recorded 2026-08-21** | S | "is active in the built bundle" failed twice under the full 793-test parallel run and passed in isolation and on the immediate rerun, with no diff. Suspect resource contention around the babel transform. Not chased yet; if it recurs, first suspect worker memory pressure, then a shared babel cache. |
