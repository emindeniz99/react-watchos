...
| **Engine alternatives behind a swappable adapter** (PrimJS, JSCOnly-jitless) | P3, experiment — **never on main until measured wins** | L | Maintainer request 2026-08-21: try engines as REPLACEABLE alternatives, not replacements — quickjs-ng stays the default and nothing is deleted; the experiment lives on its own branch and merges only if the numbers win. The seam mostly exists already (`JSRuntime` is the only thing that touches the C API; every caller goes through it), so "adapter" means: keep the engine behind that boundary and make the target swappable at build time. **(1) PrimJS** (lynx-family/primjs, Apache-2.0): Lynx's QuickJS descendant with a template interpreter — claims ~2x QuickJS on CoreMark-ish loads and keeps a QuickJS-compatible C API surface, which is exactly what makes a cheap trial possible: compile our existing embed host against its headers and run the REAL bundle. Watch the risks that decide it: C++ runtime cost on watchOS (binary size vs quickjs-ng's ~1 MB of C objects), arm64_32 support, API gaps vs quickjs-ng 0.16 (WriteObject/bytecode, columns in stacks — our symbolication depends on them), and upstream cadence (our vendor bot assumes quickjs-ng). Experiment branch: `experiment/primjs-engine` — engine fetched at a pinned commit by a build script, NOT vendored; measured against the same embed-smoke numbers (heap/boot/dispatch) before any further step. **(2) JSCOnly jitless, self-built**: the 2026-08-21 decline above stands on the *system-framework* premise; the maintainer wants the missing number recorded — build WebKit's JSCOnly target ourselves (JIT off, as watchOS would force anyway), measure the honest static-lib size + heap + boot against quickjs-ng, and let the number close or reopen the row. Expected outcome per the earlier analysis: megabytes vs ~1 MB, interpreter-vs-interpreter speed parity; but expected is not measured. Same rule: separate branch, adapter seam, no default change. |
