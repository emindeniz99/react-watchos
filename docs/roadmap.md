...
| **JSRuntime owning-queue confinement (M1)** | P1 | M | The engine's contract is "one thread per runtime": the app runtime lives on main, but WIDGET runtimes are created/evaluated/discarded on WidgetKit provider threads while the default timer path delivers `__fireTimer` on main. **Interim shipped:** intent-mode JS refuses timers loudly (`setTimer` no-op with a log — a short-lived render/intent pass could never usefully fire one). Full fix: an owning serial queue captured at `JSRuntime` init (app = main; widget = its own), all entries + timers routed through it, and the DEBUG confinement assertion generalized from "main thread" to "owning queue". |
