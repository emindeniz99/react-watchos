...
- **Design system** (2026-07 review §2.4, all three tiers) — shared layout
  ...
  → closed: watch-sim compile+test green 2026-07-04, on CI since 2026-08-07
  (`build.yml` `watchos-tests`); visual correctness stays ③.
...
- **ErrorBoundary** + on-device error banner. Richer dev overlay: the JS layer
  ...
  to show it (macOS-build-gated) → build gate cleared 2026-08-06 (status.md
  ②); still open as of 2026-09-18: the banner shows the message only and is
  DEBUG-only since 0.8.0 (`7aca6ce`) — release builds get `diagnosticsSink`
  instead.
...
(→ `js/bin/inspector-server.mts` behind `react-watchos inspector` since
2026-07-04, `3b8290c`). The `injectIntoDevTools` hook is still wired for when
a transport exists.
...
until the macOS build runs green** — that's the gate. → gate passed
2026-08-06 (watch-sim build+test, status.md ②), on CI since 2026-08-07; BLE
stays ③ device-only.
...
nowhere; release reading happens on the repo) — since 2026-09-17 (`70999b4`)
CHANGELOG.md does ship in the tarball; MIGRATIONS.md stays outside `js/`,
linked by URL from the npm page. Every entry was verified
...
extended through 0.9.0 as of 2026-09-18.)

## Graduation-review follow-ups (2026-08-06, adversarially verified)

- **Engine upgrade: quickjs-ng v0.15.1 → v0.16.1 — DONE 2026-08-06.** →
  superseded: v0.16.2 vendored 2026-09-03 (`cca5fd5`), shipped in 0.8.0
  (`6a637df`), commit-pinned 2026-09-18 (`36cdcae`). The
...
- ✅ **DONE — publish workflow** (`publishConfig.provenance` requires
  ...
  made after the bootstrap publish and 0.1.0 through 0.5.0 are on npm (0.9.1
  is latest as of 2026-09-18). The two
...
- **Theme type-safety follow-up — shipped 2026-07-05 (`5d68c4e`).** → landed
  as the typed `ColorValue` token union (types-only); the `typeof theme`
  factory was skipped on purpose, see
  [design-theme-2026-07-05.md](./design-theme-2026-07-05.md). Borrow
  Restyle's compile-time pattern for the token layer (`type Theme = typeof
  theme` + token-keyed props):
...
|---|---|---|---|
| **Digital Crown** | P0 → **shipped** (2026-06-16, `3bc0cb4`) | M | A focusable `<CrownRotation value range step onChange>` over SwiftUI `digitalCrownRotation` (+ optional crown haptic). Implement as a **component/prop, not a `useCrownRotation` hook** — it must bind to a specific SwiftUI view's crown. Today the Crown only works implicitly via `Picker`. Demo: Crown scrubs a number. → shipped 2026-06-16 as `<CrownRotation>` (component, as designed); focus claim followed in 0.7.0 — see the Focus row. |
| Gestures | P1 → **shipped** (2026-06-16/17) | M | `onLongPress` (new event kind, trivial), `DragGesture` (continuous — **coalesce/throttle**, it's the high-frequency case the cost model warns about), swipe-to-dismiss. → `onLongPress`/`onSwipe` 2026-06-16 (`c3b2d59`), `onDrag` 2026-06-17 (`e6f9110`, 4pt-quantized), row `swipeActions` 2026-06-23 (`ca38879`); swipe-to-dismiss is `onSwipe(direction)`, no separate prop. |
| Slider / Stepper / DatePicker | P1 → **shipped** (2026-06-16/17) | M | Each = prop type + `NodeView` case + render test + fixture. Slider is largely Crown+drag; Stepper is easy; DatePicker is a real primitive. → shipped 2026-06-16 (Slider/Stepper, `7551f18`) and 2026-06-17 (DatePicker, `901667c`); trap paths hardened in 0.8.0/0.9.0. |
...
| Item | Pri | Effort | Notes |
|---|---|---|---|
| **`setInterval` shim** | P0 → **shipped** (2026-06-16, `63b0f5e`) | S | The confirmed bug: `setInterval`/`clearInterval` are undefined in QuickJS, so an interval-driven update throws. Add them on top of the existing `setTimeout`→`__host.setTimer` bridge (re-arm on fire). *Async `setState` already commits via the scheduler's `setTimeout` hop* — this is the missing piece, not a scheduler redesign. Document the scheduler integration. Accept: an interval counter commits with no tap. **Land first** — unblocks Track 3's fetch/sensors. → shipped 2026-06-16; re-arm rides the one-shot host timer (shims.ts). |
...
| Native compilation as a PR gate (CX-013 reversed) | P1 → **shipped** (2026-08-20) | S | CX-013 skipped this in 2026-06 — "you don't rely on Actions for native; stays manual" — when the only native surface was a SwiftUI shell. It is now the watchOS half of the health bridge, the SwiftUI host and the widget infra, every one of which compiles to an EMPTY module off-watchOS (NF-30), so the `swift test` that runs on every push proves nothing about them. What the old rule cost is on the record: the 2026-08-07 → 08-10 run of red nightlies, found days after the commit that caused it. `build.yml` now runs on `pull_request` AND on push to main, path-filtered to `js/swift/**`, `js/plugin/**`, `app/**` and the workflow itself — a docs or renderer change still spins up no macOS runner, and `concurrency` cancels superseded runs on the same PR. Main's only ruleset (`main is protected`) requires none of the three jobs — it blocks deletion and force-push and nothing else, so a push can land with `swift-lint`/`watchos-tests`/`build` red; `release.yml` runs only the Linux JS gates (typecheck, lint, size budget, vitest against the vendored engine) before publishing, none of the three macOS jobs, so an npm release can ship with the native side red. → closed 2026-09-17 (`e946d89`, `ba21a2e`): `publish` now `needs` ci.yml, quality.yml AND build.yml run against the tag, so a release cannot ship with the native side red. The ruleset still requires nothing (audit Major 16). |
| Richer dev error overlay | P2 | S | **JS layer shipped:** ErrorBoundary forwards `componentStack`; the remote inspector rings errors (message/stack/componentStack + `console.error` tee) into an ERRORS panel. Remaining: surface it in the native on-device banner (macOS-build-gated). → gate cleared 2026-08-06; the ARCH-13 banner exists since 2026-07-16 (`53226bb`) and is DEBUG-only since 0.8.0 (`7aca6ce`); `componentStack` still not forwarded to it — open. |
...
| Live Activities / Smart Stack+ | P2 → **split**: Live Activities deferred by design (2026-06-16, see above); Smart Stack ranking + predictive `relevantContexts` shipped (2026-07-28, status.md) | M | Extends the existing widget timeline pipeline. → Live Activities deferred by design 2026-06-16 ("Live Activities — deferred" above); the Smart Stack half shipped: ranking (CX-017) and predictive `relevantContexts` (2026-07-28, ② on the 2026-08-06 watch-sim build). |
...
- The three P0s (Crown, WatchConnectivity, setInterval) are otherwise
  ...
  → all three P0s and fetch shipped 2026-06-16/17; this sequencing is history.
- **Hard gate:** every SwiftUI change is unverified until the macOS build
  ...
  on-device. → the workflow is now `build.yml` (renamed 2026-08-06); nightly
  since 2026-08-07 (green since 2026-08-10), on every native PR since
  2026-08-20 (`cb005d1`),
  and its `watchos-tests` job renders every fixture through NodeView since
  2026-09-18 (`e57b963`). Crown/gestures/WatchConnectivity are ② in status.md;
  on-device feel stays ③.
...
3. `NodeView.swift` case (+ `WidgetNodeView` in `ReactWidgetView.swift` if
   it can appear in a widget).
4. A schema assertion in `primitives.test.tsx` (the convention for added
   primitives; `render.test.tsx` is the original showcase). → since CX-024
   the primitive must also be listed in `js/codegen/schema.ts` `components`;
   `component-contract.test.ts` enforces a `case` in both interpreters.
   `primitives.test.tsx` holds the render test.
...
- **Bump `v` only for breaking changes to the shared structs** (`RNNode`,
  `RNTree`, `Published*`), and do it through `js/codegen/schema.ts` so both
...
- **Fitness APIs** — real-time heart rate, gyroscope, all-day accelerometer,
  ...
  → shipped: streams 2026-06-17, HealthKit reads 2026-07-29, widened in 0.5.0
  (2026-08-20); only the device pass remains (feature-table row "Sensors /
  HealthKit").
- **Smart Stack Relevance API + Points-of-Interest signals** — extend our
  ...
  → shipped 2026-07-28 (`poi`/`location`/`inferredLocation` clues in
  `RelevantContext`; status.md row 53).
- **Control Widget API + iOS controls shared to watchOS** — our controls
  ...
  → watch half shipped 2026-07-28 (status.md row 52); the iOS-shared half
  waits on the iOS target §7 defers.
- **MapKit** (search, routing, overlays) → a new **`Map` primitive**
  (annotations + a route) — high value for the navigation app category.
  → shipped 2026-06-17 (`Map` + `searchPOI`, annotations + polyline route;
  on-sim 2026-07-05, status.md row 63).
- **Liquid Glass** design + new SwiftUI materials/tab/split views — adopt the
  new modifiers so apps look native to 2026, not 2023.
  → shipped: `glass` 2026-06-17, `buttonStyle` glass variants 2026-07-02
  (status.md row 55; app only, no-op in widgets).
...
- **React Compiler** (React 19) auto-memoizes components, cutting re-renders
  ...
  near-term pick.
  → shipped 2026-06-16; opt-in `watchBuildOptions({ reactCompiler: true })`
  since 2026-07-02 (NF-28), js/esbuild/react-compiler.mts.
...
→ shipped: `applyUpdate` 2026-06-17, signed 2026-06-25, hardening trio
2026-07-04 (status.md row 54; docs/updates.md, docs/ota-signing.md); the
guardrail is the per-target feature contract (`requiredFeatures`,
ARCH-01/02). Open: the same-`version` replay row below (audit 2026-09-04,
Major 3).
...
- **Control Center / Action Button / Lock Screen controls** (`ControlWidget`,
  ...
  → the watchOS 11 half is live (demo `AddGlassControl`/`RemindersControl` in
  app/targets/widget/ReactWidgets.swift; status.md row 52, 2026-07-28); only
  the iOS surface still needs a host.
...
- **Downstream consumers are a minor behind.** `ctrl-a-remote` and `flarelog`
  ...
  → closed 2026-09-04 (both moved to ^0.6.0 / 0.7.0); at ^0.9.1 since
  2026-09-18.

## Re-prioritized "what's next"

1. ~~**macOS build green**~~ — **done**: `build.yml` runs on every push to
   main touching the native paths plus nightly, and the watch-sim suite
   passed locally 2026-08-10 (388 tests; docs/status.md ②). What is still
   open is the *device* pass that item 2 names, not the build.
...
5. ~~**`Map` primitive**~~ — **shipped** (`Map` in js/src/components.ts,
   NodeView's MapKit branch, render-verified on-sim 2026-07-05 with
   annotations + a polyline route; docs/status.md). Still open from this
   line: Smart Stack POI / relevance signals for widgets. → shipped
   2026-07-28 (the `poi` clue in the `RelevantContext` union — status.md;
   the 'Already shipped' section above records it).
...
→ CI-covered since build.yml (2026-08-06, green 2026-08-10 — item 1 of
'Re-prioritized'): `watchos-tests` compiles and tests the host on the
simulator, `build` runs `expo prebuild` and the React Watch scheme.
...
→ accepted in CI since build.yml (2026-08-06): the `build` job prebuilds the
demo app and builds the React Watch scheme with xcodebuild.
...
→ npm half shipped 2026-08-06 (0.1.0 bootstrap; 0.2.0 on 2026-08-07 is the
first registry-usable release; 0.9.1 is `latest` on 2026-09-18). The
SwiftPM half was superseded, not done: the host ships inside the npm tarball
(`swift/`) and the plugin wires it from `node_modules`; no remote SPM URL
exists (publishing.md still lists one as the alpha gap).
...
|---|---|---|---|
| **Production `.qbc` carries no stack positions** | **P1 → shipped 2026-08-21** | S | **Fixed and proven end to end.** `tools/qjs-compile` now writes `BYTECODE\|STRIP_SOURCE` (debug tables kept, source text still stripped): production frames go from `at d0 (<null>:0:1)` to real `file:line:column`, byte-identical to the source-parsed path. The acceptance test is the real chain, not a synthetic lookup: `js/test/qbc-symbolication.test.ts` builds a known `.tsx` through the real preset, compiles it with the real `qjs-compile`, executes the BYTECODE in the vendored engine via a minimal `JS_ReadObject`+`JS_EvalFunction` host (`tools/qjs-compile/qbc-stack.c`), captures the real `Error.stack` and resolves it through the shipped `symbolicate-core` back to the fixture's line/column/name — and restoring `STRIP_DEBUG` fails 5 of its 7 cases, headed by the `<null>` assertion. Measured cost (re-verified at ship time): +45.4 KB `.qbc` (204,979 → 250,361 B on the minified app), +36 KB QuickJS heap, +0.07 ms `JS_ReadObject`; retaining source would add +652 KB for identical stacks, so `STRIP_SOURCE` stays unconditionally. `symbolicateFrame` was extracted into `js/scripts/symbolicate-core.ts` (the CLI is a thin printer over it; old `<null>` stacks now print `[no mapping]` instead of crashing the run) (moved to `js/bin/symbolicate-core.mts` on 2026-09-17, 0.8.0, when `symbolicate` became a CLI subcommand), including a one-column call-site fallback the e2e gate itself caught: the engine reports the call opcode one column before esbuild's named identifier segment, so a nameless exact hit retries one column right, only when the neighbour is named. |
| quickjs-ng bump bot | P2 → **shipped** (2026-08-21) | M | `.github/workflows/vendor-quickjs.yml` — daily, proposes the newest release that has soaked ≥7 days. Policy is `js/scripts/pick-quickjs-release.ts` with tests: soak window, hotfix warning (a candidate is still proposed while something newer soaks, but the PR opens with a `[!WARNING]` block naming it — the day-seven vendor of a release upstream already replaced is the exact mistake), downgrade guard, draft/prerelease filter, skip-list. It resolves the candidate tag to an upstream **commit** before anything is downloaded (refusing a commit newer than the release that names it), re-vendors by that commit with the same `tools/vendor-quickjs/run.sh` a human uses, proves the tree against upstream's git objects (`verify-upstream.sh`), fails loudly if upstream's `qjs_sources` set stopped matching our four `.c` files, re-resolves the tag over the git protocol in the push job and refuses a moved tag (also when the bot branch already pinned that tag elsewhere), pushes only when the tree changed, and DISPATCHES `ci.yml` + `build.yml` on the bot branch (`workflow_dispatch` is the one event `GITHUB_TOKEN` may raise) so the gates run in the branch's own ref and cache scope — nothing from the download executes in the bot's own main-scoped run (closed 2026-09-18, after the 2026-09-18 CodeQL review; see quality-gates.md). M9 is preserved by NOT letting the bot attest: `engine-attest.yml` fails any PR touching `js/swift/Sources/CQuickJS` until a human confirms the tag→commit binding off the runner and adds `engine-digest-attested`; the check also runs `verify-upstream.sh` itself, and the label is stripped by a bot push that changes the engine tree, by any human push, and by a reopen (a push to a closed PR raises no event). **No required status checks on main today** — as of 2026-09-17 the only *branch* ruleset (`main is protected`, id 22174516; the second active ruleset, 22174510, makes release tags immutable) enforces `deletion` and `non_fast_forward` only, so `engine attest / attested` stays advisory. Marking it required is still an open owner action (docs/mac-session-checklist.md's standing owner actions, with the GITHUB_TOKEN-PR caveat recorded there); making the `build.yml` jobs required is a separate call, argued in build.yml's own header comment; `release.yml` re-runs its own gate subset instead of depending on this label. |
...
| **Where the engine sources live** (vendored vs submodule vs LFS vs fetch-at-build) | **measured & settled 2026-09-03 — keep vendoring** | S | Owner question: the engine is 2.7 MB of C in the tree, is git LFS or a submodule smarter, "the repo grows"? **The premise does not survive measurement.** Every blob that has ever existed under `js/swift/Sources/CQuickJS` — 44 of them across the 7 commits that ever touched the path — costs **1.10 MB packed**, inside a **16 MB** `.git`. The engine's entire version history is **41% the size of one checkout of it**, because git deltas C text ~4:1; the one real bump (`f308762`, 14 files, 4,755 insertions) cost **651 KB packed**. LFS would make exactly this worse: it stores every version IN FULL with no delta, so the same bump costs 2.62 MB — **4× more**, moved from a free place to a metered one. Three constraints kill the alternatives outright, each verified rather than assumed. **(1) SwiftPM cannot fetch C sources at build time, at all**: SE-0303 sandboxes build-tool and prebuild plugins with no network, `allowNetworkConnections` exists only for command plugins a human invokes (never during `swift build`), the manifest is sandboxed too, and SE-0305's `binaryTarget` accepts only an `.xcframework` or an executable-only `.artifactbundle` — i.e. a prebuilt binary we would host, the exact inversion of the CHECKSUMS.sha256 + engine-attest posture. `npm postinstall` is not the escape hatch either: pnpm 10+ blocks dependency lifecycle scripts by default, so a download-on-install engine silently would not download for most consumers. **(2) `npm pack` packs the working tree and knows nothing about gitlinks or LFS filters** — measured by experiment: with the submodule checked out the tarball carried the sources; with it uninitialized `npm pack` shipped **one file**, exit 0, no warning. `actions/checkout`'s `lfs:` input defaults to **false**, so one forgotten flag publishes 130-byte pointer files as "C sources" and the failure lands in a stranger's Xcode build. **(3) GitHub source archives exclude LFS objects by default**, and a contributor without git-lfs gets a repo that looks complete and cannot build. Judged from three independent lenses (consumer, supply-chain, maintenance) — all three ranked it vendor > fetch-at-publish > submodule > LFS, unanimously. **Reopen trigger, measured not felt:** the packed cost of the CQuickJS path crosses ~100 MB, or one bump exceeds ~5 MB packed. Neither is within an order of magnitude today. If tarball weight ever becomes a real consumer complaint (2.7 MB of 5.15 MB unpacked; the whole tarball still compresses to 1.17 MB), the lever is a separate npm package for the engine — still vendored in git. Follow-ups the panel converged on: assert the engine is actually IN the published tarball (release.yml already packs and extracts it for the gitleaks scan — verify CHECKSUMS.sha256 there, since `prepublishOnly` does not run the vendor-integrity test) — **done 2026-09-03** (release.yml 'Verify the vendored engine in the tarball', `02c37bb`); pin the vendoring input to an upstream **commit SHA** — **done 2026-09-18** (`Upstream commit:` in VERSION.md, `archive/<commit>.tar.gz`, `verify-upstream.sh` fetches by SHA and fails if the tag no longer names it, `resolve-tag.sh` is the git-protocol check a human and the push job share); checksum `VERSION.md` (it carries the provenance and is one of two files the manifest omits; `vendor-integrity.test.ts` now at least pins the commit line's shape) — open; make the `engine-digest-attested` label non-sticky across pushes — **done 2026-09-18** (the bot strips it when its push changes the engine tree; engine-attest strips it on `synchronize` and `reopened`); document `git clone --filter=blob:none` for anyone who wants LFS's one real benefit without LFS — open. Rejected on inspection: switching the vendor input to upstream's `quickjs-amalgam.zip` — it collapses four auditable translation units into one 3 MB file. |
| **`uses:` self-repository syntax — zizmor vs actionlint** | **parked 2026-09-03 with a revisit trigger** | S | zizmor 1.30.0 audits for GitHub's `$/` self-repository ref (shipped 2026-07-30: resolves to the running commit, no `@ref`, and unlike `./` it counts as pinning for policy purposes). actionlint 1.7.12 — the newest release — rejects `$/` for reusable-workflow calls outright. Both tools gate the push hook, so release.yml's `uses: ./.github/workflows/*` lines (vendor-quickjs.yml had two more until 2026-09-18, when the bot moved to dispatching its gates) cannot satisfy both. Kept `./` and suppressed the zizmor finding inline with the reason, because actionlint is the tool that models what GitHub will PARSE, and a `uses:` GitHub refuses fails the whole file before any job starts (the graduation-lesson-13 failure mode). Nothing is lost meanwhile: `$/` resolves to the same commit `./` does here. **Upstream is already handled — do not file anything:** rhysd/actionlint#711 (2026-07-30) asks for it and #732 (2026-09-01, ArloL, 17 files) implements it, still open. **Revisit trigger: an actionlint release containing #732.** Note the wait may be long — actionlint's last release is v1.7.12 (2026-03-30) and its last commit on main is 2026-04-19, so the project is quiet. Recorded also because the experiment that would settle it empirically is not runnable here: pushing a `$/` probe branch requires `--no-verify` to get past the very hook under discussion, and this repo forbids that — the workaround is self-enforcing until upstream moves. |
| **Engine ahead of the last release — a check, not a habit** | open (recorded 2026-09-17) | S | The bot now commits `fix(deps): vendor quickjs-ng <tag>`, so a bot bump is released on its own. A HAND bump committed as `chore(deps)` re-strands silently: main carries a newer `js/swift/Sources/CQuickJS/VERSION.md` than npm ships and nothing says so — the v0.16.2-on-main / v0.16.1-on-npm state the audit found (2026-09-04, Major #8). Wanted: a job in ci.yml that reads the `Vendored:` line of VERSION.md at HEAD and at the last `react-watchos-v*` tag and fails when they differ on a push to main whose release-please PR does not carry the change — or, cheaper, the same comparison in release.yml's `gate` step, warning when the tag it is about to publish ships an older engine than main. Not done in the fix that changed the bot's commit type, because that fix removed the only stranding seen so far; this row is here so the hand-bump case is tracked rather than assumed away. |
| **Same-`version` OTA replay** | **shipped 2026-09-18** (scheme `v3`, commit `ad96697`; MIGRATIONS.md `0.9.x → 0.10.0`) | M | Shipped as designed below, with three decisions worth naming: the mark (`react.ota.sequenceHighWater`) is raised at STAGE-accept, not at boot, so the stage→relaunch window is closed too; boot and the crash-loop known-good restore stay sequence-blind so rollback survives; equal is accepted (`>=`, identical-bytes re-stage). Follow-ups, not lost: no `OTAConfig.shippedSequence` seed (a value set higher than what is served would brick OTA until the next publish — the fresh-install window stays bounded by `expiresAt`), and no JS pre-download filter (`isFresherRelease` still downloads a refused replay every check; `getUpdateState().sequenceHighWater` is the hook). Original design: the signed message was `v2:<keyId>:<version>:<expiresAt>:<js>`; `releaseId` is an unsigned content hash with no order, and `VersionPolicy` compares `version` alone. So an earlier bundle signed at the current compatibility `version` can be re-served from the manifest URL and installs as a "fresh" release; the high-water mark cannot see it (audit 2026-09-04, Major #3). docs/ota-signing.md now says so and names the signed expiry as the only bound. Closing it is a **wire change**: bind a monotonic, signed ordinal into the message (a `v3` scheme with a publish sequence or timestamp between `version` and `expiresAt`), persist the highest seen next to `otaHighWater`, refuse anything below it in `OTABootSequencer.stage`, and update `signManifest`/`ota:sign`, `UpdatePlan.signedMessage` and every fixture together — pre-release, so no `v2` compatibility branch — not because nothing has shipped (eight versions have), but because the signature covers the scheme prefix: a `v3`-verifying binary rejects a `v2`-signed bundle as unsigned and keeps its shipped bundle, which is the fallback the OTA path already has. Not a half measure to ship from a CI branch; it wants its own change with the Swift and JS halves and their tests in one PR. |
