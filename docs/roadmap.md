...
|---|---|---|---|
| **Production `.qbc` carries no stack positions** | **P1** | M | **The real find, and it makes 2026-08-21's source maps half-useless.** `tools/qjs-compile/qjs-compile.c:114` writes with `JS_WRITE_OBJ_STRIP_SOURCE \| JS_WRITE_OBJ_STRIP_DEBUG`, and the watch app + widget **prefer `.qbc` over the JS** — so the artifact that actually runs in production has had its `pc2line`/filename tables stripped, and its `Error.stack` cannot name a position for any map to resolve. The map we now emit therefore covers the dev/JS path only. Fix direction: keep `STRIP_SOURCE` (no original text in the artifact — that part is right), drop `STRIP_DEBUG`, and measure A (today) / B (source stripped, debug kept) / C (nothing stripped, diagnostic only) on `.qbc` bytes, QuickJS heap, load+eval ms. **Measure the column question FIRST**, because it decides whether the rest is worth anything: the shipped bundle is minified onto ONE line, so `line` is always 1 and every bit of information is in the COLUMN. If quickjs-ng's serialized debug info does not round-trip columns, retaining debug buys nothing on its own and the answer becomes esbuild's `lineLimit` (wrap the bundle at ~500 chars so line numbers mean something again; cost is ~1 byte per line, no semantic change). Acceptance: an end-to-end fixture that throws in a known `.tsx`, runs the REAL production `.qbc` in the vendored engine, and resolves the frame back to that file/line/column through the private map. A synthetic `originalPositionFor` unit test does not count. |
...
| Bundle composition report from the esbuild metafile | P2 | S | `analyzeMetafile` over app + widget: largest modules, anything the widget drags in from the app half, tree-shaking defeats, `sideEffects` drift, dev-only code that survived. Cheapest remaining size lever — real dependency elimination beats syntax-level shaving. |
| ES target sweep (ES2020 → 2021/22/23) | P3 | S | quickjs-ng accepts far more than ES2020, so the target is set by our compatibility promise, not the engine. Measure JS + `.qbc` bytes per target, prove the vendored engine parses and runs each, then decide — but the published package documents ES2020 for QuickJS, and ~1-3% is not obviously worth narrowing that contract. Document the trade either way. |
| Terser as a final pass after esbuild | P3 | M | Measure, do not adopt on faith: esbuild is typically within a few percent of Terser, and the second minifier costs a source-map composition step (`sourceMap.content`) that is exactly where symbolication fidelity gets lost. Adopt only on a repeatable, meaningful win. Property mangling stays **forbidden** regardless (`mangleProps`, `mangle.properties`) — the wire protocol and `__host` bridge are property names. |
| JS obfuscation | P3, expected reject | S | Measure once so the answer is on the record, then almost certainly decline: on a watch, bytes/heap/boot outrank IP friction, and `.qbc` with source stripped already removes the readable text. Never a security boundary; no secrets behind it. |
