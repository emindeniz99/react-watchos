...
- [~] Phase 4 DX (CX-011/012/020 + DX-1..7), Phase 5 — partial:
  ...
  - [ ] remaining: **DX-7 full smoke** (pack → install into a clean Expo fixture → `expo prebuild` → assert the watch target *builds*) — a heavy macOS-CI integration job, deferred there. DX-4 (the tarball ships the right files) + the in-repo plain-`expo prebuild` verifications (products linked, plist merged on demo + example) already cover the consumable-package essence; the from-tarball build is the remaining end-to-end gate. Phase 5 — separate.
