...
- [x] **Foundation: ARCH-01** — feature manifest (complete — the remaining scope was the generated install table, which shipped as CX-023):
...
- [x] **Foundation** ARCH-01 (feature manifest schema/codegen — verifiable) → ARCH-03 (app/widget bundles) → ARCH-04 (transactional OTA); absorbs CX-002/CX-003/CX-004/CX-007. *(All three shipped; ARCH-04 closed 2026-07-27 — its last sliver, the explicit `bundleReady` health signal, landed as slice 6.)*
...
- [x] **ARCH-04** (transactional OTA state machine — **complete**):
  ...
  - [x] slice 6 — **explicit `bundleReady` health signal** (ARCH-04's last sliver; closes the item). Until now "healthy" meant "committed one tree", which cannot tell a correct screen from a blank one or an `ErrorBoundary` fallback — and, worse, a bundle that renders and *then* reliably dies resets the counter every launch, so `maxOTABootAttempts` is unreachable for exactly the failure the counter exists to catch. New `OTAConfig.healthSignal: OTAHealthSignal` (`.firstCommit` default / `.explicit`) lets an app require the bundle's own `markUpdateHealthy()` — a `via:"invoke"`, `feature:"ota"`, `targets:["watch"]` schema entry (no `bridgeProtocol` bump: pre-release, so no old-binary/new-bundle pair can exist) resolving `Promise<void>` that never rejects, driving the SAME `markHealthy(bootedRecord:)` the commit path does. No timer and no clock seam: an unconfirmed bundle simply never clears the counter, so the EXISTING crash-loop guard rolls it back after 3 launches — the CodePush `notifyAppReady` / `systemd-bless-boot` / Android `markBootSuccessful` shape (see [prior-art.md](./prior-art.md)). Two carve-outs keep `.explicit` from misfiring, both pure and Linux-tested in `OTABootSequencer.commitBlesses(bootedRecord:)`: a SHIPPED boot self-blesses (else a counter left by a dropped OTA rolls back the *next* staged bundle), and a bundle that is already the known-good snapshot self-blesses (else flipping the flag on an existing install condemns a healthy bundle that predates the API). `commitBlesses` is evaluated ONCE at the end of `load()` into `commitBlessesHealth` — it reads the known-good record off disk, which must never touch the 10-20 commits/sec path (the same reason `markedHealthyThisBoot` exists). `getUpdateState` also gained `healthSignal` + `bootAttempts` so telemetry can see "fleet on explicit, this device one launch from rollback". Docs: health-signal section in [ota-signing.md](./ota-signing.md), precedent table in [prior-art.md](./prior-art.md). +6 sequencer tests (`swift test` 252), +2 JS tests (suite 428), codegen byte-identical, typecheck/biome/embed-smoke clean. **Accepted limit:** under the DEFAULT `.firstCommit`, the render-then-die case remains uncaught; the review's "bounded stabilization interval" is deliberately not shipped as an unconditional default because watchOS glance sessions would roll back healthy bundles (a bare timer is the wrong tool — see the codex review's close-out). **Review hardening (`2a33d9f`):** the staged-bundle boot budget is per-artifact — `persist()` resets the crash-loop counter when the staged record replaces a different bundle, so a successor no longer inherits its predecessor's failed attempts — and an explicit bless arriving while the bundle is still evaluating is parked and applied once `load()` knows the booted record, so it can no longer fall through to the shipped carve-out.
...
- [x] **ARCH-06 — widget publications bound to state + release revisions (2026-07-27).** A widget payload used to carry only `publishedAt`, so a consumer could tell how OLD it was but never whether the state it describes had moved — and a mutation landing inside the freshness window is invisible to a timestamp. Shipped in four slices (`30f1b8d`/`baa130d`/`603bb67`/`1b34a84`, tests `a09667b`). **The revision:** ONE monotonic per-App-Group counter, not per-key (a payload derives from whatever set of keys its `render()` chose to read — per-key would need dependency tracking through React) and not a hash (O(state) per publish, and hashes don't order). It reuses the ARCH-05 primitive — a `CoordinatedCounterStore` pointed at its own `revision/` subdirectory via a new `subdirectory:` parameter, because keys become file names and a bundle calling `Storage.counterAdd("state", …)` would otherwise write the revision itself. **Which writes bump: all of them**, matching the decision `intents.ts` already made (it republishes on ANY Storage write); a widget-relevance opt-out would contradict that and add public surface for a speculative win. Write amplification is bounded by BATCHING, not by relevance filtering: the pure `StateRevisionTracker` bumps on the first write since the last publication, so it's one coordinated claim per mutation batch, not per `Storage.set`. **Ordering is the load-bearing detail** — the bump happens BEFORE the write in both `setItem`/`counterAdd` closures (app + widget extension), so a crash between them leaves the revision AHEAD of the data and the payload reads *stale* (spurious recompute, safe); write-then-bump would let a payload read *current* while state had moved, which is the bug itself. `Storage.set` keeps its signature: "every committed mutation returns a revision" is satisfied natively inside the bridge, where JS can neither forget it nor mis-order it; the new `stateRevision` schema method (`feature:"storage"`, both targets, no invoke, no `HostFeatures` change) only READS, and it is the first ZERO-arg direct host method — the trampoline generator no longer binds `argv`/`argc` when there are none. **The wire:** `PublishedWidgets` gains `stateRevision: Int` (required) + `releaseId: String?`; `v` and `publishedAt` already covered the review's other two bullets. `releaseId` is optional because "producer unknown" is a real axis (a runtime booting precompiled bytecode has no source to hash) and nil NEVER rejects — otherwise a shipped-bytecode fleet would recompute forever. Both extension gaps closed so nil is rare: the widget runtime now injects `globalThis.__bundleReleaseId` (the app has done so since CX-025; the extension never did) and sets the id on the shipped-BYTECODE path, which is exactly the configuration where app and extension genuinely run different bundles (app on an unproven OTA record, extension on shipped). **Sampling discipline:** the revision is sampled at RENDER START, before any `render()` reads state, mirroring the `cacheEpoch` supersede check `renderFreshTimelines` already uses — a concurrent write then leaves the payload stamped older and detectably stale, whereas sampling after the render would certify data the payload doesn't contain. **The consumer side:** `WidgetSnapshot.isCurrent` is unchanged and still tested; the new pure `WidgetSnapshot.freshness` layers above it returning `.current/.staleRevision/.foreignRelease/.expired` with revision → release → time precedence (a stale revision MUST beat a live `reloadAfter`: an author's "good until 3pm" is a claim about the clock and cannot know the user changed the state at 2pm). Mismatch → the recompute path that already existed (`renderFreshTimelines`); **never a blank complication** — if the fresh render fails the stored payload is still shown. `newestPayload` moved out of the watchOS-only provider into Support (so it is Linux-tested at all) and orders by revision with `publishedAt` as tie-break: "written last" was the wrong question, since an in-extension render that started before an intent's write can land after it. The extension persists its own booted release id to the App Group so a TimelineProvider can name the release it reads with **without booting an engine** — the boot is the cost the check exists to avoid. **Reconciliation:** `reconcileWidgets()` (published revision ≠ live → evaluate the new `__republishWidgets` global) runs once per boot off the first healthy commit and on every foreground, where a control intent handled in the extension may have mutated shared state while the app was away; a new `widgets` diagnostic subsystem leaves evidence, since a reconcile means a publication was lost. Verification: `swift test` 252 → **272** (freshness precedence matrix incl. the acceptance's crash-window case, `StateRevisionTracker` batching, revision-counter namespace isolation + monotonicity, `newestPayload` ordering, store round-trip with stamps, the zero-arg trampoline), JS 428 → **434** (stamping, sample-once-at-render-start, `__republishWidgets`, and a lost-publication simulation with a throwing host), codegen byte-identical, typecheck/biome clean, embed-smoke heap 2.1 MB / boot 41.2 ms, bench perDispatch 0.559–0.668 ms across three runs against the ~0.52 ms ARCH-09 baseline — unchanged by construction, since the extra hop is one call per `renderWidgets`, not per dispatch. **Honest guarantee** (written into the codex review's close-out): monotonic + cross-process-atomic bump, bump-before-write ⇒ every crash between mutation and publication reads stale and never current, and a residual sub-millisecond cross-process window where A samples after B's bump but before B's UserDefaults write lands — B's own publication supersedes it, so the system converges. That is the Decision's "eventually consistent, inconsistency observable and recoverable", not linearizability. **Follow-ups, deliberately not built:** (1) a 2-phase `pending`/`committed` revision pair to close that window (doubles coordinated claims per batch); (2) a `Storage.set(key, value, {widgetRelevant:false})` escape hatch if all-writes-bump ever shows up in battery numbers (additive, decidable later); (3) skipping the `WidgetCenter` reload when a republish differs only in `publishedAt` — a real refresh-budget saving, but new comparison logic — **shipped 2026-07-28** as the pure `WidgetPublishGate.shouldReload`, which decodes both payloads and compares `v`/`stateRevision`/`releaseId`/`widgets`/`controls` (never the JSON strings, never `publishedAt`), fails open toward freshness on any doubt (nil/undecodable previous, any field diff), and gates the WAKE only — the save, and with it the freshness bookkeeping, is unconditional at both publish sites (app host + widget extension, where it gates `invalidateCache()` too). The state write (a UserDefaults key) and the bump (a coordinated file) stay two stores: watchOS App Groups have no cross-key transaction, and making them one means the transactional document store already deferred at ARCH-05. **Review hardening (`88a2914`):** the batch invariant is now "the revision moves on the first write after the last SAMPLE-or-publication" — sampling closes the batch at both bridge sites and `reconcileWidgets()` closes it on observing a foreign publication, so a mid-render write is detectably stale even mid-batch and a cross-process publish cannot leave this process stamping current over moved state; the extension's render-only pass CONSUMES (not closes) the batch via a one-shot flag, so pure re-renders converge while intent-path writes still bump; `stateRevision` moved to `feature:"core"` so an ARCH-07 policy denying `storage` cannot poison freshness; and `reconcileWidgets()` no-ops without an App Group store. All three blocker scenarios re-proven load-bearing (pre-fix wiring fails, fixed passes) by an execution harness; swift 275 / JS 436.
...
- [x] **ARCH-08 — `WatchRoot.dispose()` + queue-confined `JSRuntime.shutdown()` (2026-07-27).** Shipped **re-scoped**, and the re-scope is the finding: the review's `RuntimeSession` object graph (executor + policy + registries + cancellation scope + diagnostics behind `createWatchApplication(config)`) was **not** what remained. A reload already destroys the whole QuickJS context (`boot()` → `runtime = nil` → `makeRuntime()`), so **every** process global and module binding resets by construction, and every async settle has been generation-guarded since CX-008 (including `js.bridge.commit`'s `decodeQueue` hop). Building the object would have been a rename of `boot()`'s reset block against the highest-risk wiring in the project — rule 2/3. Three things were genuinely unsolved, and those shipped in four slices (`281f5b9`/`4bd6d86`/`fcbf564`/`6945499`, Swift `deb46f1`). **(1) Same-context double-mount.** `runApp` installed `__dispatchEvent`/`__pushNativeEvent`/`__inspect` and nothing removed them; a second call built a second root, repointed the globals at it, and left root #1 **fully mounted** — its effect-registered listeners still in the shared `nativeEvents` table, so every native push fanned out to both trees while only the new one was reachable, the stale one committing into a host nobody read. `WatchRoot.dispose()` now unmounts (React runs the effect cleanups, which is what actually releases listeners/sensor tokens/timers), then uninstalls **only** the globals still pointing at its own closures (the `stopFn === stop` identity pattern from `inspector.ts`), and every later `render`/`dispatchEvent`/`runSync`/`inspect` throws instead of mutating a dead tree; a module-level `activeRoot` makes a second `runApp` **throw** rather than supersede (auto-dispose-with-a-warning was the alternative and was rejected — it makes a real double-mount look like it worked). **Deliberately NOT in dispose:** the `listeners`/`intents`/widget registries (module-scope `registerIntent`/`registerWidget` run BEFORE any `runApp`, and the widget/intent entrypoints have no `runApp` at all — `unregisterAll*` stay the separate explicit reset), the Swift-owned `__host*` globals, the inspector (process-level dev tool spanning reloads by design), and in-flight invoke/fetch/generate promises (id-correlated to native work still running; their watchdogs bound them and cancelling native work is `boot()`'s job). **(2) A module-singleton desync bug, in-tree.** `inspector.ts`'s `diagnosticsTapped` latched true on the first `startInspector` and was never reset — but the tap it guarded is a `registerNativeListener` subscription, so **any** `unregisterAllNativeListeners()` silently revoked it and every later `startInspector` reported **zero diagnostics forever**: the dev tool going quiet exactly when something had gone wrong. The unsubscribe is now held and released by `stop()`, so a restart re-subscribes; no test-only reset API. The `console` tee stays one-way on purpose (it always calls the original first, and nothing can revoke it the way the listener table revoked the tap) — documented in place. **(3) Off-queue `JS_FreeRuntime` — the memory-safety item, and the reason the prior design doc's "no failing scenario today" is refuted.** `JSRuntime.deinit` did the right work but ran on whatever thread dropped the last reference. For the app runtime that is main by luck; for the **three private-queue runtimes** (OTA validate, OTA compile, widget) it is a staging/WidgetKit thread. A staged bundle that arms a `setTimeout` at module scope leaves a live `DispatchSourceTimer` on the validate queue, so dropping the validator there mutated `pendingTimers` concurrently with that source's handler and called `JS_FreeRuntime` while `__fireTimer` could be re-entering the context — a data race **plus a use-after-free reachable from a signed-but-hostile OTA bundle**. `public func shutdown()` now does the cancel/free inside the owning queue (inline when already confined, so `deinit` can't `sync` onto itself; `sync` hop otherwise), is idempotent via `didShutdown`, and `deinit` calls it behind a DEBUG `assert(didShutdown || isOnOwningQueue)` so a future off-queue owner **traps at its own call site** instead of relying on a hop that can only serialize the race, not undo it. Every JS entry refuses loudly after shutdown (`JSError.shutdown` / `onError("shutdown", …)`) instead of dereferencing the freed context. Call sites: `defer { validator.shutdown() }` / `defer { compiler.shutdown() }` in the OTA closures (defer, so a **throwing** evaluate still tears down), `WidgetIntentRuntime.deinit`, and `boot()` before `runtime = nil` — which also makes the QuickJS free **ordered** relative to `sensors.stopAll()`/`bluetooth.resetPendingForReload()`/the media stops instead of implicit in ARC. Root cause of why this was never testable: `isOnOwningQueue` asked `owningQueue === DispatchQueue.main`, and `DispatchQueue.main` has **no stable object identity on Linux** (fresh wrapper per access), so it answered "off queue" on the main thread there; whether the runtime owns main is now recorded at init — Apple behavior unchanged, Linux now matches it. Also `ReactWatchModel` gets a DEBUG `deinit` cancelling `devTask`: the dev-reload poll held no reference back (no leak kept the model alive) but nothing cancelled it, so it kept waking every 2 s to poll the dev server after the model died. **(4) The bulk of the diff: test hygiene.** Eight files each hand-wrote the same `afterEach` and each got a different subset right — a different mix of deleted globals, a different mix of `unregisterAll*`, and **none** unmounted its root; vitest's per-file isolation is the only reason nothing was red. `test/helpers.ts` gains `mountApp()` + `resetApp()`, all eight bespoke blocks are gone, and every `runApp` call site in the suite is now disposed. `resetApp` deliberately does NOT delete the three `runApp` globals — `dispose()` owns those, so a call that skipped `mountApp` fails loudly on the next mount instead of being papered over. Verify: **swift test 275 → 279** (timer cancelled across an off-queue shutdown, idempotency, six refused entry points, quiet release from any thread), **JS 436 → 445** (dispose contract incl. the effect-cleanup that stops native fan-out, identity-checked uninstall, the double-`runApp` throw, slot release on a throwing first render, and the inspector re-tap), typecheck/biome clean, embed-smoke green. Both negative checks done by reverting the fix: the timer case fails as `call __fireTimer after JSRuntime.shutdown()` without the cancel, the inspector case fails without the unsubscribe, and the deinit assertion traps when a private-queue test drops its runtime unshut. `ReactWatchHost.swift`/`WidgetIntentRuntime.swift` are `#if os(watchOS)` → `swiftc -parse` clean + hunk review here; they need `pnpm test:swift:watch` on a sim/device. **Deferred, with reason:** upgrading the BLE reload path from *drop* to *reject loudly* (the surviving live bullet from `design-arch-08-runtime-session.md`) was evaluated and NOT folded in — the sensor half of that item does not exist (`SensorBridge` has no invoke correlation at all; sensor start/stop are direct host calls), and the BLE half is not the advertised two-liner: `resetPendingForReload()` runs INSIDE `boot()` while the outgoing runtime is still live, so rejecting there re-enters the dying context and runs the old bundle's `.catch` handlers mid-teardown — the exact hazard the dispose design rules out. The abandoned Promise is cosmetic (the context is gone; nobody is awaiting), so this wants its own decision and test, not a fold-in. **Amendment (2026-07-28) — the recorded known limit is closed: the OTA→shipped fallback now boots a FRESH runtime.** What the limit was: `__disposeActiveRoot` releases ROOT-owned resources, but an OTA bundle that throws partway through its module body has already run arbitrary code in that context and almost none of it hangs off the root — armed `setTimeout`s, `registerWidget`/`registerIntent` entries (which run BEFORE any `runApp` and are explicitly out of dispose's scope, see above), a started inspector with its deliberately one-way `console` tee, and every module singleton it initialised. `load()` then evaluated the SHIPPED bundle into that same QuickJS context, so a signed-but-hostile or merely broken bundle left residue in the shipped app's runtime — the recovery path handing the recovery bundle the wreckage. The fix: `load()` latches whether an OTA artifact began evaluating (both eval closures set it), and the `evalShipped` closure, if that latch is set, calls the new `replaceRuntimeAfterPoisonedOTA()` — `tearDownGeneration()` (extracted verbatim from `boot()`, so there is ONE teardown: fetch cancel, `sensors.stopAll()`, BLE pending drop, audio/speech/extended-runtime stop, `JSRuntime.shutdown()`, generation bump) then `installFreshRuntime()` (extracted likewise from `boot()`: `makeRuntime()` + `runtime =` + `installHostCapabilities` + the DEBUG inspector URL). Both extractions exist so the retry cannot drift from the first boot — a hand-copied second wiring sequence would silently lose a step, and the copy that lost it would be the one running the recovery bundle. **The four traps, each handled explicitly and commented in place.** *(1) The parked blessing.* `handleMarkUpdateHealthy` parks into `pendingMarkHealthy` whenever `jsReady` is false, which is the whole of `load()` — so a poisoned bundle that called `markUpdateHealthy()` before throwing would leave a flag the shipped bundle's boot then applies, and `load()` could not tell "the shipped bundle confirmed itself" from "the dead bundle's confirmation blessed a boot it never survived". Reset on the failure path. Today's blast radius is small and stated honestly: the sequencer has already zeroed the crash-loop counter by then, so `markHealthy` would be a no-op — but it would also latch `markedHealthyThisBoot`, suppressing the shipped bundle's own first-healthy-commit blessing. *(2) Boot-attempt accounting.* Traced through `OTABootSequencer.boot()`: the `.runOTA` branch increments BEFORE `evalOTA` and, on the throw, `dropActive()` + `setOTABootAttempts(0)` before falling through to `evalShipped()` (the crash-loop-rollback branch does the same after its `setOTABootAttempts(1)`). So the counter is already reconciled when the fallback runs, the shipped eval happens exactly once and INSIDE the sequencer, and the host must not touch the counter at all — it doesn't. The sequencer needed no change and got none; what it needed was the properties the host now relies on to be pinned, hence two new Linux tests asserting the eval ORDER (`["ota", "shipped"]`, and `["ota:good()", "shipped"]` for the rollback path) and the counter value observed FROM INSIDE `evalShipped` (0, from a non-zero start). *(3) `commitBlessesHealth` recompute.* Already correct by construction and now verified + commented rather than re-implemented: only the `.ranOTA` outcomes populate `bootedOTARecord`, this path produces `.ranShipped`, so the existing unconditional `commitBlesses(bootedRecord: nil)` at the end of `load()` self-blesses under BOTH policies (pinned by `testExplicitPolicyStillSelfBlessesShippedBoots`) — and it runs before any commit can be processed, since commits cross `decodeQueue` and `load()` holds main. *(4) The generation bump.* Needed, and decided from the code rather than assumed: `boot()` bumps once per boot and this retry lives INSIDE one `boot()` call, so without a second bump the poisoned runtime's parked callbacks would carry exactly the generation the fresh runtime is created under and every CX-008 guard would wave them through. `shutdown()` kills the old context's timers, but nothing cancels a URLSession completion, an `MKLocalSearch` callback, a notification-add callback or a StoreKit task the poisoned module body started — those are HOST closures that outlive the context and settle by id, and the fresh bundle's id spaces restart at 1, so a dead bundle's id can name a live promise in the new runtime. `tearDownGeneration()` supplies the bump, and it lands before `makeRuntime()` because `js.onError` captures the generation at construction. **The widget extension needs no equivalent change**, checked not assumed: `WidgetIntentRuntime` builds exactly one `JSRuntime` in `init?` and a failing bundle fails the whole initialiser (`return nil`, `deinit` shuts it down), so no path evaluates a DIFFERENT bundle into a context a failed one touched — in particular a failed known-good OTA bundle does NOT fall back to shipped in place; the caller gets nil and the next request builds a runtime from scratch. Its two bytecode→source retries re-evaluate the same bundle and their residue dies with the runtime moments later. Documented in `loadBundle`; no behavior touched. `__disposeActiveRoot` survives, now covering exactly the bytecode→source retries (`evalOTA` and `loadShipped`), where the same bundle is re-parsed into the context its own failed `.qbc` load touched; its doc comment says so instead of advertising a known limit. *Recorded residual (2026-07-28, review):* under NON-enforced keys, `evalOTA`'s bytecode-throw → source retry evaluates the source artifact into the context the failed `.qbc` load touched — inside the dev/unsigned carve-out only (enforced keys skip untrusted bytecode entirely), so it is a dev-mode sliver; wants its own finding if the carve-out's threat model ever changes. Verify: **`swift test` 298 → 300** on Linux (the two sequencer order/budget tests), `swiftc -parse` clean on `ReactWatchHost.swift` + `WidgetIntentRuntime.swift`, and — because `-parse` does not type-check an inactive `#if os(watchOS)` block — the new constructs (a mutable local latch captured by all three eval closures, `try` inside the branch that swaps the runtime, the two extracted helpers) were additionally re-created in a standalone Swift-6 harness and `swiftc -typecheck`ed clean. Negative check on the new tests: deleting the sequencer's post-throw `setOTABootAttempts(0)` (both catches) makes the budget observed from inside `evalShipped` 2 on the plain path and 1 on the rollback path, failing both new tests while their ORDER assertions still pass — so neither is vacuous. **Owed at the next Mac build:** this whole path is watchOS-only and cannot be executed here — the fallback itself is unreachable on Linux, so the trace is line-by-line reasoning, not a run. `xcodebuild -scheme ReactWatchHost-Package -sdk watchsimulator` is the compile gate, and the behavioural proof (stage a bundle that arms a timer and then throws; confirm the shipped boot sees no timer, no stale registrations, and a clean inspector) is a sim/device run.
- [x] **ARCH-11 — invoke-channel shape contract + a runtime-closed error set (2026-07-27).** Shipped **re-scoped P1 → P2**, and as with ARCH-08 the re-scope is the finding. The review's premise — "without generated method IDs/types … it can become a second stringly typed bridge" — was **measured, not assumed**: every JS-declared shape was checked against its Swift producer/consumer (`DeviceSnapshot.current()` vs `DeviceInfo`, `StoreKitBridge.products` vs `IAPProduct`, `handleGetUpdateState` vs `UpdateState`, `handleSearchPOI` vs `POIResult`, `permissionStatus` vs the 5 `NotificationPermission` values) and the structural drift found was **zero**. Two of the five acceptance boxes were already shipped (disposed-session/unknown-id rejection on both sides; the preserved streaming channels), and three are **rejected outright** — `deadline`, `sessionId`, cancellation, backpressure — with the reasons on the review heading. What the missing contract HAD let through was four adjacent defects, and those are what this pass fixed, in five slices. **(1) The one completely unguarded surface: the JS caller literals.** `codegen.test.ts` pins the schema ↔ Swift `handleInvoke` router in both directions, but `invoke("keychainGett")` in `js/src` compiled, type-checked, linted and passed all 56 test files — surfacing only as a runtime `UNKNOWN_METHOD` on a physical watch (and in the widget runtime, whose rejecter is name-only **by design**, so this one guard covers both runtimes). `test/invoke-routing.test.ts` scans `js/src/**/*.ts{,x}` for `invoke<T>?("name")` literals and asserts set-equality with the schema in both directions, plus a third case pinning the regex itself against the awkward call shapes so neither direction can go vacuous. **(2) The closed error set was closed by TYPE ONLY — the live repeat-offender.** JS's `settle()` did `parsed.code as InvokeErrorCode`, an unchecked cast, and the Swift reject sites passed bare `String`s: two independent literals, one union type, one cast, and nothing forcing them together. It shipped twice — `playAudio`'s `"AUDIO_FAILED"` (caught by the 2026-07-02 self-review cycle) and then `getCurrentLocation`'s `"LOCATION_UNAVAILABLE"`, still live and meaning no `if (e.code === "UNAVAILABLE")` could ever match a denied location. `InvokeErrorCode` is now a **Swift enum** next to `InvokeErrorJSON`, taken by `make(code:message:)`, so instance #3 is a compile error at the reject site; all 30-odd reject sites converted (dispatcher, capability handlers, `BluetoothBridge`, `PhoneConnectivity.SendError`, the widget rejecter). JS `settle()` validates against a `Record<InvokeErrorCode, true>` (indexed, so `"constructor"` can't pass) and degrades an unrecognized code to `INTERNAL` **keeping the original spelling in the message**, so an older binary's code stays diagnosable rather than erased. `LOCATION_UNAVAILABLE` split by what it actually meant: `OneShotLocation.Failure` gains `.denied` vs `.unavailable`, `.denied`/`.restricted` → `PERMISSION_DENIED` (the user said no; Settings will help, re-prompting won't), a CLError or unknown authorization state → `UNAVAILABLE`. **(3) The shapes themselves.** `invokeShapes` in `codegen/schema.ts` declares 13 request + 7 result shapes, referenced by `HostMethod.request`/`.response`, with an explicit `"opaque"` sentinel for `sendToPhone`/`updateApplicationContext`/`transferUserInfo` — whose payload IS the consuming app's own JSON and which Swift hands to WCSession without reading a field; declaring that is what keeps *undescribable by design* distinguishable from *nobody declared it*. Codegen emits TS interfaces + an `INVOKE_SHAPES` table into `src/generated/wire.ts`, and Codable mirrors into the **TEST target** (`Tests/ReactWatchTests/Generated/InvokeShapes.swift`) — deliberately not `ReactWatchCore`, because the shapes are a contract and not a runtime dependency, and Core is linked by both binaries including the widget extension. The gate runs both ways: `test/invoke-contract.test.ts` drives every payload-carrying wrapper through a recording host and writes the payload it **really** serialized plus the result it really resolved to `Fixtures/invoke-*.json` (extending the `contract-fixture.test.tsx` → `WireContractTests.swift` pattern, so the REQUEST half is real serializer output and never a hand-authored copy — the RESPONSE half is the result the wrapper resolved from a TS-literal mock reply, bound to the public interface by the `Exact<>` assertion; honest but weaker, and a native→wire producer-key gate is the recorded follow-up), asserting completeness against `INVOKE_SHAPES` in both directions; `InvokeContractTests.swift` decodes all 24 with the generated struct, requires the three opaque ones to still be JSON **objects** (which is exactly what `PhoneConnectivity` casts to), and runs the two payloads with a shipped Linux-reachable decoder through the **real** one (`NotificationPlan`, `UpdatePlan`). On the JS end an `Exact<A, B>` assertion — mutual assignability **plus** identical key sets, so width subtyping can't hide an extra field — binds each result shape to the public interface callers already use. **(4) Three decodes that turned a malformed payload into a plausible success.** `scheduleBackgroundRefresh`'s `afterMs ?? 0` became "wake me RIGHT NOW" — a battery-budget request the developer never made, spending one of watchOS's ~hourly grants immediately and, for the documented reschedule-from-your-own-handler pattern, in a loop; `getProducts`' `productIds ?? []` resolved an empty product list for a typo'd key, indistinguishable from "the App Store knows none of these ids" — the one outcome a paywall must not render silently; `searchPOI` resolved `"[]"` for an unparseable payload, i.e. "no coffee near you". All three now go through the shared `rejectInvalid`, which the other 12 payload-carrying handlers already used. `searchPOI` **splits** the two cases its single guard had merged: an empty QUERY still resolves `[]` per maps.ts's documented contract; only the malformed REQUEST rejects. **(5) A lifecycle promise that resolved before its session ran** — separate commit, because no envelope generator catches it: the contract broken is temporal, not structural. `extendedRuntime.ts` promised "resolves when it becomes running; rejects if the system declines (already active, or unsupported session type)" while the handler resolved `"null"` the instant `start()` returned. `WKExtendedRuntimeSession.start()` is asynchronous, and `ExtendedRuntimeBridge.start()`'s "a session is already live" early-return still reported success — so the most common real failure (an app missing the Info.plist runtime-session reason, which the system invalidates immediately) looked like a successful start. The invoke id + generation is now parked and settled from `extendedRuntimeSessionDidStart` / `didInvalidateWith`, the same shape as the remote-push registration parking in the same file: `running` resolves, `invalidated` rejects `UNAVAILABLE` with the system's reason, stale generations drop (CX-008), and `boot()`'s `stop(silent:)` detaches the delegate so a reload produces no callback at all. `start()` returns `Bool` because "already active" is the one refusal that produces **no** delegate callback and would otherwise hang the parked invoke to its 30 s watchdog. **Deliberately NOT done, and the biggest call in the item:** option (i), generated `Codable` request structs decoded in `handleInvoke` — the review's own shape. Its marginal benefit is compile-time rather than test-time enforcement for **9 methods** that already have hand-written guards; its cost is a 31-signature migration that supersedes two already-unit-tested hand-written decoders, needs a stringly escape hatch for the three opaque payloads anyway, and drags `saveUpdate`'s decode back onto main (it lives inside `OTASequencer.stage`, off-main, by M5). Large cost, measured drift of zero. The trade-off is recorded on the `invokeShapes` doc block so it is revisitable with evidence. **The honest limit of what shipped**, stated in the same place: the 31 handlers keep their own decoders, so the gate proves *JS payload ≡ schema* and *schema ≡ public TS types* — it does **not** force a handler to read the field names its method declares. Verify: **JS 449 → 459** (routing guard ×3, contract fixtures ×5, `settle()` degrading an unrecognized code and refusing an inherited Object key ×2), **swift test 282 → 288** (5 contract-fixture cases + the enum's wire spellings pinned against the TS union's six), `codegen --check` clean with all four generated files regenerating byte-identically, typecheck/biome clean, `swift format lint` unchanged at its 42 pre-existing warnings, embed-smoke green. Negative checks by breaking each new guard: renaming one JS literal to `keychainGett` fails **both** routing directions; renaming `speak`'s `text` to `txt` fails the Swift fixture decode with `keyNotFound`; adding a field to device.ts's `DeviceInfo` without the schema fails typecheck twice (the fixture literal and the `Exact<>` binding). `ReactWatchHost.swift`/`CapabilityBridges.swift`/`PhoneConnectivity.swift`/`BluetoothBridge.swift`/`LocationBridge.swift`/`WidgetIntentRuntime.swift` are `#if os(watchOS)` → `swiftc -parse` clean + hunk review here; they need `pnpm test:swift:watch` on a sim/device, and the extended-runtime delegate settle is the one behaviour that can only be exercised there.
...
- **SD-5 → RuntimeSession (ARCH-08).** A session object owning
  ...
  *(Shipped 2026-07-27, RE-SCOPED — the session OBJECT was not built and was
  not what remained: the fresh-context-per-reload already provides the
  isolation. What shipped is `WatchRoot.dispose()` + a single-active-root
  guard, the inspector diagnostics-tap fix, and a queue-confined
  `JSRuntime.shutdown()`. See the build-progress log entry above.)*
- **SD-1 → generated typed envelopes (ARCH-11).** ~~`{requestId, methodId,
  payload, deadline, sessionId} → result|error|cancelled` with
  timeout/cancellation~~ *(2026-07-27: RE-SCOPED and shipped. The wire shape
  `__host.invoke(id, method, payloadJson)` → `__resolveInvoke`/`__rejectInvoke`
  ALREADY is `{requestId, methodId, payload} → result | typed error`, and
  `invoke` is itself a direct schema method whose C trampoline is generated —
  changing its arity would touch the QuickJS boundary for no gain. What shipped
  is the compile-time layer over that unchanged wire: schema-declared
  request/result shapes with cross-language fixtures, a runtime-closed error
  enum, three silent-default decodes turned into INVALID_REQUEST rejections,
  and a JS-caller-literal routing guard. `deadline`, `sessionId`, cancellation
  and backpressure are recorded as WONTFIX with reasons on the review heading.
  See the build-progress log entry above.)*
...
- **ARCH-04 expansions — OTA validation + rollback (P0).** Validate a candidate
  ...
  + rotation.
  *(Shipped 2026-07-05→27 — see the build-progress log entry above; slice 6
  closed the `bundleReady` sliver.)*
...
- **ARCH-06 — bind widget publications to state + release revisions (P1).**
  Payloads carry the source state revision and producing release id; providers
  detect stale revisions / foreign releases and recompute.
  *(Shipped 2026-07-27 — see the build-progress log entry above.)*
