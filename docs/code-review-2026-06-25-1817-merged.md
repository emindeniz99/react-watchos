...
- [~] **Foundation: ARCH-01** — feature manifest (in progress):
  ...
  - [x] slice 3 — capability gate wired end-to-end. JS: `UpdateManifest` gains `requiredFeatures`/`minBridgeProtocol`; `checkForUpdate`/`fetchAndApplyUpdate` gate BEFORE download against `globalThis.__hostFeatures` (→ `appUpdateRequired`, no download/crash), +3 tests. Native: `ReactWatchHost` injects `__hostFeatures` (= `HostFeatures.watch`) + `__bridgeProtocol` at boot (verified on watchOS build). **Remaining:** save-time enforcement (`CapabilityGate` in `saveUpdate`, needs `requiredFeatures` in the UpdatePlan payload — defense-in-depth) + ARCH-02 build-time derivation of a bundle's `requiredFeatures`.
