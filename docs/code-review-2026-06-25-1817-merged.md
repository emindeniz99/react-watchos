...
- [~] **CX-022** — typed results for silent fallible native ops:
  ...
  - [ ] remaining ops — **BLE connect/write** (complex: CoreBluetooth state machine, currently event-driven via `ble(json)`+`ble.state`/`ble.notify` pushes; needs a device to runtime-verify — a real redesign, not a mechanical port) and **scheduleNotification** (low value: `UNUserNotificationCenter.add` rarely errors, and the id is generated in JS so callers already have it synchronously — the Promise-result shape fits awkwardly). Both flagged for a deliberate pass rather than forced.
...
- [~] **CX-017** — per-entry `TimelineEntryRelevance(score:duration:)` is **already wired** (`ReactWidgets.swift` maps each JS entry's `relevance` → the Smart Stack ranking signal). Remaining: the *predictive* `relevantContexts → WidgetRelevance` mapping (date/location surfacing) — decoded into `relevantContexts` but not yet applied; it's a watchOS-version-specific API best done on a device. Flagged, not forced.
