...
- [x] **Foundation** ARCH-01 (feature manifest schema/codegen — verifiable) → ARCH-03 (app/widget bundles) → ARCH-04 (transactional OTA); absorbs CX-002/CX-003/CX-004/CX-007. *(All three shipped; ARCH-04's one open sliver is the explicit `bundleReady` health signal.)*
...
- [~] **ARCH-04** (transactional OTA state machine — open sliver: explicit `bundleReady`):
