...
| 17 | CX-007 / CX-025 | Version is both rollback gate AND freshness → non-breaking fixes never ship; two manual sources | ✅ REAL (CX-007 done; **CX-025 designed**) | [update.ts:81](../js/src/update.ts) `version >`; [config.mjs:15](../js/scripts/config.mjs) | CX-007 (keyId/rotation) shipped. CX-025 spec written — [design-cx-025-release-freshness.md](./design-cx-025-release-freshness.md): `releaseId` for freshness (gate stays `version`); the cross-language hash primitive is **proven** (JS FNV-1a == Swift `ContentHash`), deferred only on the load-flow integration + freshness-semantics change (a focused pass, not a blind multi-file edit). | M |
