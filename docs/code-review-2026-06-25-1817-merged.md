...
- [~] **CX-022** — typed results for silent fallible native ops:
  ...
  - [ ] remaining ops — build `scheduleNotification`, BLE connect/write, connectivity send on the invoke channel (now that it exists, each is a JS `invoke(...)` + a native routing case).
