...
- [~] **CX-022** — typed results for silent fallible native ops:
  ...
  - [ ] remaining ops — `scheduleNotification`, BLE connect/write, connectivity send. **Planned: introduce the generic `invoke(id, method, payload)` channel (SD-1) as a `refactor` before these**, migrate fetch/generate/saveUpdate/permission onto it, then build BLE/connectivity on it — per research (Capacitor/TurboModule prior art: one correlation-keyed channel beats ~10 hand-rolled `__resolve*` globals; pays off at ~3rd–4th op).
