...
- [~] **CX-022** — typed results for silent fallible native ops:
  ...
  - [ ] remaining op — **BLE connect/write** (complex: CoreBluetooth state machine, currently event-driven via `ble(json)`+`ble.state`/`ble.notify` pushes; needs a real BLE device to runtime-verify — a redesign, not a mechanical port).
