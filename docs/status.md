...
|---|---|---|
| ① **Logic-tested** | Pure logic unit-tested off-device | `pnpm test` (410 JS, vitest — incl. the codegen-drift check, green on Linux since the swift-format implicit-stdin fallback) + `swift test` (227 green, Linux/macOS: Core/Support/Runtime — the SwiftPM package first compiled + tested on **Linux** 2026-07-09, Swift 6) — counts drift; treat as point-in-time (2026-07-16) |
