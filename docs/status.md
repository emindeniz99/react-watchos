...
| MapKit POI search (`searchPOI` → MKLocalSearch), `getCurrentLocation`, full-screen + auto-fitting `Map` | ③ | invoke routing drift-tested ① (`codegen.test`); `handleSearchPOI`/`handleGetCurrentLocation` ②; **on-sim ③ (2026-07-05, Ultra 3):** the `/map-search` "Places" demo — a full-screen map with a floating search field — centers on the watch's location (`getCurrentLocation`; verified by setting the sim to NYC → map opened on New York), biases the search there, and its controlled camera zooms to fit the results (searched "Park" → 15 NYC parks framed; falls back to SF when location is unavailable) |
