...
| MapKit POI search (`searchPOI` → MKLocalSearch) + full-screen `Map` | ③ | invoke routing drift-tested ① (`codegen.test`); `handleSearchPOI` ②; **on-sim ③ (2026-07-05, Ultra 3):** the `/map-search` "Places" demo — a full-screen map with a floating search field — searched "Park" and dropped 15 pins across SF read live from MKLocalSearch |
