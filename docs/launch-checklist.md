...
— by owner decision (see `engine-attest.yml`'s own gap). A red `ci.yml` or
...
| R3 | First release cut (release-please PR merged, tag + GitHub release + npm publish job green) | ✅ | Happened, repeatedly, not just once: **8 releases published** (0.1.0 through 0.7.0 — `gh api repos/.../tags`, checked 2026-09-17), `release.yml`'s publish job green on each, most recently 0.7.0. |
